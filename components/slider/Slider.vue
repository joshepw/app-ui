<template>
	<div :class="['slide-wrap', {' transition' : transition && !touching}]" v-swipe:start="_swipeStart"
		v-swipe:move="_swipeMove" v-swipe:end="_swipeEnd">
		<div class="slide-inner"
			v-bind:style="{ transform: 'translate3d('+(-mutableActiveIndex*100+diff/width*100)+'%, 0, 0)' }"
			v-transition-end="_transitionEnd">
		...
		<div class="bullets">
			<div :class="['bullet', {'active': mutableActiveIndex === i || (mutableActiveIndex === amount - 1 && i === 1) || (mutableActiveIndex === 0 && i === amount - 2)}]" v-for="i in bullets" :key="i"></div>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			_swipeMove(point, diff, time) {
				if ((this.mutableActiveIndex === 0 && diff.x > 0) || (this.mutableActiveIndex === this.amount - 1 && diff
						.x < 0)) {
